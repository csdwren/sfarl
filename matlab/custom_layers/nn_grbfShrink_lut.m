function [y,dzdw,J] = nn_grbfShrink_lut(x,weights,means,sigma,dzdy,varargin)

%GRBFShrink shrinkage using an RBF-mixture with truncated Gaussian basis
...
%
%   Y = NN_GRBFShrink_LUT(X,WEIGHTS,MEANS,SIGMA) applies the function
...
%
%   [DZDX, DZDW] = NN_GRBFShrink_LUT(X, WEIGHTS,MEANS,SIGMA, DZDY) computes
...
%
%   NN_GRBFShrink_LUT(...,'OPT',VALUE,...) takes the following options:
%
...
err_msg=['nn_grbfShrink_lut:: The first dimension of weights must match ',...
...
misc.checkEq(opts.data_mu(1,1)+means(1),opts.origin,...
...
if nargin <= 4 || isempty(dzdy)
  ...
  % bin.
  y = L(opts.Idx+idx_l*szW(1)).*(1-wh) + L(opts.Idx+idx_h*szW(1)).*wh;
   
  if nargout > 2
    J = LD(opts.Idx+idx_l*szW(1)).*(1-wh) + LD(opts.Idx+idx_h*szW(1)).*wh;
  else
    J = [];
  end  
