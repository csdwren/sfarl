function [y,dh1,dh1_data,dh2,dh2_data,ds1,ds1_data,ds2,ds2_data,dw,dw_data,da,dl,J,J_data,M,M_data] ...
    ...
    rbf_weights,rbf_weights_data,rbf_means,rbf_precision,blur_kernel,alpha,lambda,dzdy,varargin)
...
if nargin < 18 || isempty(dzdy)
    ...
    if opts.conserveMemory
        ...
        %% fidelity term
        pad_blur = floor(size(blur_kernel,1)/2);
        kx = vl_nnconv(x,blur_kernel,[],'stride',opts.stride, 'pad',pad_blur,'dilate',1,opts.cuDNN);
        res_input = kx - Obs; % residual image
        ...
        y_data = vl_nnconv(y_data,rot90(blur_kernel,2),[],'stride',opts.stride, 'pad',pad_blur,'dilate',1,opts.cuDNN);
