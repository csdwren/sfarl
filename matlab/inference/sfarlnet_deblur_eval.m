function res = sfarlnet_deblur_eval(net, x, dzdy, res, varargin)
...
%       opts.rbf_weights,opts.rbf_weights_data,opts.alpha,opts.lambda}},...
      ...
      if doder
        [res(i+1).x,~,~,~,~,~,~,~,~,~,~,~,~,res(i+1).aux{1},res(i+1).aux{2},res(i+1).aux{3},res(i+1).aux{4}] = ...
          sfarlnet_deblur(checkInput(res(i).x), ...
          ...
          opts.netParams.blur_kernel, l.weights{11}, l.weights{12}, [], 'conserveMemory', ...
        ...
        res(i+1).x = sfarlnet_deblur(checkInput(res(i).x), opts.netParams.Obs, ...
